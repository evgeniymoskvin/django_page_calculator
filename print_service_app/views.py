...
def get_report_xls(objects_tasks: list):
    file_xlsx_path = os.path.join(settings.BASE_DIR, 'print_service_app', 'xlsx', 'GurnRas_Print&Scan.xlsx')
    file_path_to_export = os.path.join(settings.BASE_DIR, 'print_service_app', 'xlsx', 'export.xlsx')
    ...
    for task in objects_tasks:
        ...
        try:
            ...
            who_change_status_to_done = last_recording_done.emp
        except Exception as e:
            print(e)
        ...
        row = ['',  # № п/п
               ...
               '',  # Скан
               pages_info_task.a0x3_color,
               f'{who_change_status_to_done.last_name} {who_change_status_to_done.first_name[:1]}.{who_change_status_to_done.middle_name[:1]}.'
               ]
        page.append(row)
        try:
